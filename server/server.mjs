...
const startServer = (server, port, address) =>
  new Promise((resolve, reject) => server.listen(port, address, err => (err ? reject(err) : resolve())));

export default async function start({ port = 5000, address = '0.0.0.0' } = {}) {
  const app = express();
